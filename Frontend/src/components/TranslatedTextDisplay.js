...
const TranslatedTextDisplay = ({ isDarkMode, onToggleDarkMode }) => {
  ...
  const [sessionId, setSessionId] = useState(() => sessionStorage.getItem('signify-session-id') || null);

  // Listen for session_id in API responses (from window for simplicity)
  useEffect(() => {
    const handler = (e) => {
      if (e.detail && e.detail.session_id) {
        setSessionId(e.detail.session_id);
        sessionStorage.setItem('signify-session-id', e.detail.session_id);
      }
    };
    window.addEventListener('signify-session-id', handler);
    return () => window.removeEventListener('signify-session-id', handler);
  }, []);
  ...
  // Reset session on delete translation
  ...
  return (
    <div className={`translated-container ${isDarkMode ? 'dark-mode' : ''}`}>
      ...
      <button className={`btn-17 ${isDarkMode ? 'dark-mode-button' : ''}`} onClick={handleDeleteTranslation} disabled={translatedText.length === 0}>
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 -2 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-trash-2"><path d="M3 6h18" />
      ...
      <button className={`btn-17 ${isDarkMode ? 'dark-mode-button' : ''}`} onClick={handleTranslate} disabled={loading || !combinedText}>
        {loading ? <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-loader">
          <path d="M12 2v4" /><path d="m16.2 7.8 2.9-2.9" /><path d="M18 12h4" /><path d="m16.2 16.2 2.9 2.9" /><path d="M12 18v4" /><path d="m4.9 19.1 2.9-2.9" /><path d="M2 12h4" /><path d="m4.9 4.9 2.9 2.9" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 -2 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-languages"><path d="m5 8 6 6" /><path d="m4 14 6-6 2-3" /><path d="M2 5h12" /><path d="M7 2h1" /><path d="m22 22-5-10-5 10" /><path d="M14 18h6" /></svg>} {loading ? 'جارٍ الترجمة...' : 'ترجم النص'}
      </button>
      {translation && (
        <div className={`translated-result ${isDarkMode ? 'dark-mode-result' : ''}`}>
          ...
          <svg xmlns="http://www.w3.org/2000/svg" onClick={() => handleSpeak(translation, targetLanguage)}
              width="35" height="35" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="lucide lucide-volume-2">
